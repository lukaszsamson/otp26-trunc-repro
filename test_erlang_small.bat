...
call erl -noshell -eval "io:setopts(standard_io, [{encoding, latin1}]), {ok, Data} = file:read_file(\"small.txt\"), file:write(standard_io, Data), erlang:halt()." > out.txt

REM Compute and display the MD5 hash for in.txt
certutil -hashfile small.txt MD5
